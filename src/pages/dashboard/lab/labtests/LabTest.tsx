...
import scss from "@/styles/LabTest.module.scss"
...
const patients: { patientId: string; reportGenerated: boolean; selectLabTest: string }[] = [];
const LabTest = () => {
    ...
    const createReport = (patientId: string) => {
        ...
        // Redirect the user
        Router.replace({
            pathname: '/dashboard/lab/labtests/filltest',
            query: { patientId },
        });
    };
    const generateReport = async (patientId: string, selectLabTest: string) => {
        ...
        if (patientData) {
            await Router.replace({
                pathname: `/dashboard/lab/labtests/${patientData.selectLabTest}`,
                query: { patientId, selectLabTest },
            });
    ...
    const printReport = async (patientId: string, selectLabTest: string) => {
        ...
        if (patientData) {
            await Router.replace({
                pathname: `/dashboard/reports/printreport/${patientData.selectLabTest}`,
                query: { patientId, selectLabTest },
            });

        } else {
            console.error('Patient data or Lab Test is not available');
        }
    };
    ...
    const GenerateReportCell = (params: GridCellParams) => {
        ...
        useEffect(() => {
            const fetchGeneratedReports = async () => {
                const q = query(collection(db, "generatedlabreports"), where("patientId", "==", params.row.patientId));
                const querySnapshot = await getDocs(q);
                const reports = querySnapshot.docs.map(doc => doc.data());
                if (reports.length > 0) {
                    setReportState('generated');
                } else {
                    const q = query(collection(db, "labreports"), where("patientId", "==", params.row.patientId));
                    const querySnapshot = await getDocs(q);
                    const reports = querySnapshot.docs.map(doc => doc.data());
                    if (reports.length > 0) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const reportSnapshot = await getDocs(collection(db, "labreports"));
            ...
            const combinedData = patientData.map((patient) => {
                ...
                return {
                    ...
                    selectLabTest: report?.selectLabTest,
    ...
    const columns = [
        ...
        { field: 'selectLabTest', headerName: 'Test Name', width: 300 },
        {
    ...
    return (
        <div className={scss.container} >
            <h1 className={scss.title}>Lab</h1>
            <h2 className={scss.subtitle}>Lab Tests</h2>
            ...
            <div className={scss.dataGrid}>
                <DataGrid
                    ...
                    rows={data?.rows.filter((row) =>
                        ...
                        (row.TypeOfExamination == 'Lab Test')
                    ) || []}
                    columns={columns || []}
                    sortModel={[
                        {
